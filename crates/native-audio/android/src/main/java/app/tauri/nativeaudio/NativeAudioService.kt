...
class NativeAudioService : MediaSessionService() {
    ...
    /// A crossfade swaps which ExoPlayer is the active one, and the session
    /// player is rebuilt around it. The notification holds its own reference,
    /// so without this it would keep drawing — and controlling — the deck that
    /// already faded out.
    private val deckListener: (androidx.media3.common.Player) -> Unit = { player ->
    ...
    override fun onCreate() {
        ...
        NativeAudioRuntime.addDeckListener(deckListener)
    }
    ...
    override fun onDestroy() {
        NativeAudioRuntime.removeDeckListener(deckListener)
